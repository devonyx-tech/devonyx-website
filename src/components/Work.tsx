					...
					<div className="max-w-2xl">
						...
						<Reveal delay={200}>
							<p className="mt-5 text-lg text-ink-soft">
								Real projects, real outcomes. A look at what Devonyx delivers
								for founders across the USA, UAE, Netherlands, New Zealand &
								India.
							</p>
						</Reveal>
					</div>
							...
							<div className="group flex h-full flex-col overflow-hidden rounded-3xl border border-hairline bg-surface transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-brand/10">
								<div className="p-6 pb-0">
									<div className="mb-4 flex items-center justify-between">
										<div
											className={`flex items-center gap-2 rounded-full bg-linear-to-r ${project.gradient} px-3 py-1 text-xs font-semibold text-white`}
										>
									...
									<div className="mb-4 rounded-2xl border border-hairline bg-paper p-4">
										<div className="mb-3 flex items-end justify-between">
											<p
												className={`font-display text-3xl font-medium bg-linear-to-r ${project.gradient} bg-clip-text text-transparent`}
											>
												{project.metric}
											</p>
									...
									<div className="mb-5 grid grid-cols-3 gap-2">
										{project.kpis.map((kpi) => (
											<div
												key={kpi.label}
												className="rounded-xl border border-hairline bg-surface px-2 py-2.5 text-center"
											>
								...
								<div className="flex flex-col p-6 pt-0">
									...
									<div className="mt-4 flex flex-wrap gap-2">
										{project.tags.map((tag) => (
											<span
												key={tag}
												className="rounded-full border border-hairline bg-paper px-3 py-1 text-xs text-ink-muted"
											>
												{tag}
											</span>
										))}
		...
		<div className="relative h-20">
			...
			<div className="relative flex h-20 items-end gap-1.5">
				{bars.map((b, _i) => (
					<div
						key={`bar-${b}`}
						className="flex-1 rounded-t bg-black/5"
						style={{ height: `${b}%` }}
					/>
				))}
			</div>
