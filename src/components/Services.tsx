				...
				<div className="max-w-2xl">
					...
					<Reveal delay={100}>
						<h2 className="mt-5 text-balance font-display text-4xl font-medium tracking-tight text-ink md:text-5xl">
							Three pillars.{" "}
							<span className="serif-accent text-brand-dark">Every stage</span>{" "}
							of your growth.
						</h2>
					</Reveal>
					<Reveal delay={200}>
						<p className="mt-5 text-lg text-ink-soft">
							Whether you need a product built, an audience converted, or a
							brand people remember — we cover the full journey from idea to
							impact.
						</p>
					</Reveal>
				</div>
				...
				<div className="mt-14 grid gap-6 lg:grid-cols-3">
					{pillars.map((pillar, i) => (
						<Reveal key={pillar.id} delay={i * 120}>
							<div
								className={`group relative flex h-full flex-col rounded-3xl border border-hairline bg-surface p-8 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${pillar.glow}`}
							>
								<div className="mb-6 flex items-start justify-between">
									<div
										className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-linear-to-br ${pillar.gradient} shadow-lg`}
									>
								...
								<ul className="mt-6 flex flex-col gap-2.5">
									{pillar.services.map((service) => (
										...
										>
											<span
												className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-linear-to-br ${pillar.gradient}`}
											>
