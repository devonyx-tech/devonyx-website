    ...
    <div className={depth === 0 ? 'mt-2' : ''}>
      {blocks.map((block) => {
        ...
        switch (block.type) {
          ...
          case 'heading_1':
            return <h2 key={block.id}><RichText value={rt(block.heading_1 as Record<string, unknown>)} /></h2>
          case 'heading_2':
            return <h3 key={block.id}><RichText value={rt(block.heading_2 as Record<string, unknown>)} /></h3>
          case 'heading_3':
            return <h4 key={block.id}><RichText value={rt(block.heading_3 as Record<string, unknown>)} /></h4>
          ...
          case 'toggle':
            return (
              <details key={block.id}>
                <summary><RichText value={rt(block.toggle as Record<string, unknown>)} /></summary>
                {children}
              </details>
            )
          case 'quote':
            return <blockquote key={block.id}><RichText value={rt(block.quote as Record<string, unknown>)} /></blockquote>
          case 'code':
            return (
              <pre key={block.id}>
                <code>{rt(block.code as Record<string, unknown>).map((t) => t.plain_text).join('')}</code>
              </pre>
            )
          case 'divider':
          ...
          case 'image': {
            ...
            return (
              <figure key={block.id}>
                <img src={src as string} alt={(img?.caption as Array<{ plain_text: string }>)?.map((c) => c.plain_text).join('') || ''} loading="lazy" />
                {img?.caption?.length > 0 && <figcaption>{(img.caption as Array<{ plain_text: string }>).map((c) => c.plain_text).join('')}</figcaption>}
              </figure>
            )
          }
          case 'child_page':
